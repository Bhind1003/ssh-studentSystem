...
public class checkImage extends ActionSupport {
    public String checkImg() throws Exception {
        ...
        // 绘制任何图形之前 都必须指定一个颜色
        graphics.setColor(getRandColor(180, 240));
        ...
        for (int i = 0; i < 4; i++) {
            ...
            sb.append(c);
            // 将c 输出到图片
            graphics2d.rotate(theta, x, 27);
            graphics2d.drawString(String.valueOf(c), x, 27);
            graphics2d.rotate(-theta, x, 20);
            x += 30;
        }

        // 将生成的字母存入到session中
        ServletActionContext.getRequest().getSession().setAttribute("checked", sb.toString());
